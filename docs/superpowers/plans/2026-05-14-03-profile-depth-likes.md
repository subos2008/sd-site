...
- Task 21: In `src/features/search/components/ProfileCard.tsx`, wrapped the `setPhotoUrl(null)` reset (when `card.primary_photo_path` becomes null) in `queueMicrotask` to satisfy ESLint's `react-hooks/set-state-in-effect` rule. Behaviourally identical and still cancellable via the effect cleanup.
- Task 22: Added `banner.placeholder.title` and `banner.placeholder.body` keys to `src/i18n/en/notifications.json`. Reason: `BannerHost` interpolates `banner.${kind}.{title|body}`, and `notification_kind` includes both `'like'` and `'placeholder'` (Plan 03 has the placeholder kind as a forward-compat hook). Without the keys i18next would render the raw `banner.placeholder.title` literal for any placeholder notification. The new keys are intentionally generic ("Notification" / "Something happened.") since `placeholder` is itself a forward-compat slot.
- Task 24: In `e2e/likes-and-filters.spec.ts`, replaced the spec's `getByRole('button', { name: /^like$/i })` / `/^unlike$/i` selectors with `aria-pressed="false"` / `aria-pressed="true"` locators inside the card. Reason: the actual `LikeButton` renders `♡ Like` / `♥ Unlike` (heart symbol prefix), so the strict `/^like$/i` regex won't match the accessible name. The `aria-pressed` reflects liked state and is set by `LikeButton`, so the selector is both semantically correct and stable to label changes.
- Post-Plan-03 verification fixup: the verification gate flagged two issues. (1) `shared/db-types.ts` was missing the `touch_last_active` RPC line because the Task 11 migration (`20260514000011_rpc_heartbeat.sql`) landed without regenerating. Fix: ran `pnpm gen:types` and committed the single-line addition. (2) Vitest count was 42 (floor ≥45). Fix: added two new test files — `src/features/search/__tests__/hooks.test.tsx` (2 tests for `useSearchFilters` URL parse + write round-trip via a Probe component) and `src/features/likes/components/__tests__/LikeButton.test.tsx` (2 tests: aria-pressed rendering when unliked, and MSW-asserted `like_profile` RPC call on click). Final count 46 tests across 25 files. The second `useSearchFilters` test captures the setter via `useEffect` (not direct render-time assignment) to satisfy `react-hooks/globals` lint.
