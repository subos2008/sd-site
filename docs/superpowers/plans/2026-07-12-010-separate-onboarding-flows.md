...
- **Post-Task-8: no global error surface exists (user decision, 2026-07-16).** The repo has no react-query `QueryCache`/`MutationCache` `onError` and no render error boundary. Filed issue #2 for the proper global surface; as a stopgap, added local error surfacing to Details/Interests steps so no onboarding mutation error is silent. (Also filed issue #1: report frontend errors to Sentry.)
- **Task 12 expanded to fix a broken `seed:dev` (2026-07-16).** Task 11 surfaced, and psql confirmed, that `service_role` has no table grants on the app tables (same newer-Supabase-CLI regression as Task 2's `authenticated` fix, which didn't cover `service_role`). So `scripts/seed-dev-users.mjs`'s direct `.from('profiles').update(...)` failed with `permission denied` — silently, since it never checked the error — leaving seeded profiles stuck at `pending_onboarding`. Task 12 therefore also adds a migration granting `service_role` its table privileges (standard Supabase posture; server-side-only key, not a user-facing surface) and makes the seed check its update error. The E2E (Task 11) already sidesteps `seed:dev` by self-seeding via real onboarding RPCs, so tests don't depend on this.
